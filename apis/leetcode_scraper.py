...
class LeetcodeScraper:
        ...
        def scrape_single_operation(operation):
            ...
            try:
                response = requests.post(
                    self.base_url, json=json_data, stream=True, verify=False
                )
    ...
    def _scrape_single_global_ranking_page(self, page_num, only_user_details=True):
        ...
        try:
            response = requests.post(
                self.base_url, json={"query": query}, stream=True, verify=False
            )
    ...
    def scrape_all_global_ranking_users(self):
        first_response = (
            self._scrape_single_global_ranking_page(1, only_user_details=False) or {}
        )
        total_leetcode_global_ranking_users = first_response["totalUsers"]
        users_per_page = first_response["userPerPage"]
        total_global_ranking_pages = (
            total_leetcode_global_ranking_users // users_per_page
        )
        print(
            f"Total Leetcode users: {total_leetcode_global_ranking_users}",
            f"Users per page: {users_per_page}",
            f"Total pages: {total_global_ranking_pages}",
            sep="\n",
        )
